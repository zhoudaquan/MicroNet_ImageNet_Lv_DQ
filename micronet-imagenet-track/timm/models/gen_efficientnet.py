...
from .WSConv_utils import WSConv2d
...
class DepthwiseSeparableConv(nn.Module):
    ...
    def __init__(self, in_chs, out_chs, kernel_size,
        ...
        self.bn2 = None if folded_bn else nn.BatchNorm2d(in_chs, momentum=bn_momentum, eps=bn_eps)
...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, in_chs, out_chs, kernel_size,
                 ...
                 bn_momentum=_BN_MOMENTUM_PT_DEFAULT, bn_eps=_BN_EPS_PT_DEFAULT,
                 folded_bn=False, padding_same=False, drop_connect_rate=0.,sampling=False):
        ...
        if sampling:
            print("Sampling based on RW code: True")
            self.conv_pw = WSConv2d(in_chs, mid_chs, kernel_size=1, bias=False, multiplier = 4/2, rep_dim=0, use_coeff=False)
        else:
            print("Sampling based on RW code: False")
        ...
        if sampling:
            # print("sampling projecting conv...")
            self.conv_pwl = WSConv2d(mid_chs, out_chs, kernel_size=1, bias=False, multiplier = 4/2, rep_dim=1, use_coeff=False)
        else:
...
class GenEfficientNet(nn.Module):
    ...
    def __init__(self, block_args, num_classes=1000, in_chans=3, stem_size=32, num_features=1280,
                 ...
                 folded_bn=False, padding_same=False, first_conv_stride = 2):
        ...
        self.conv_stem = sconv2d(
            ...
            padding=_padding_arg(1, padding_same), stride=first_conv_stride, bias=folded_bn)
...
def _gen_mobilenet_v2_cifar(channel_multiplier, num_classes=10, **kwargs):
...
@register_model
def mobilenetv2_100_cifar10(pretrained=False, num_classes=10, in_chans=3, **kwargs):
    default_cfg = default_cfgs['mobilenetv2_100']
    model = _gen_mobilenet_v2_cifar(1.0, num_classes=num_classes, in_chans=in_chans, **kwargs)
    model.default_cfg = default_cfg
    if pretrained:
        load_pretrained(model, default_cfg, num_classes, in_chans)
    return model
...
@register_model
def mobilenetv2_100_cifar10_wsv2(pretrained=False, num_classes=10, in_chans=3, **kwargs):
    from .mobilenetv2_WS_scheme_2_with_param_coeff_2 import MobileNetV2
...
def efficientnet_b0_dq_ori(pretained=False, num_classes=1000, in_chans=3, **kwargs):
    from .efficientnet_pytorch.model_backup import EfficientNet
    return EfficientNet.from_name(model_name = 'efficientnet-b0')

@register_model
def efficientnet_b7_dq(pretained=False, num_classes=1000, in_chans=3, **kwargs):
    from .standard_efficientnet import EfficientNet
    return EfficientNet.from_pretrained(model_name = 'efficientnet-b7')
